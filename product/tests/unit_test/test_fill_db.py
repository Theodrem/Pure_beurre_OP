...
DATA_PRODUCT = {'products':
                    {0:
                         {'product_name':
                              'Milk chocolate',
                          'url':
                              'https://fr.openfoodfacts.org/produit/25387643/milk-chocolate-dairyfine',
                          'nutrition_grades': 'e', 'selected_images':
                              {'front': {'small':
                                             {'fr':
...
class TestMangement(TestCase):
    ...
    def test_insert_category(self):
        ...
        with patch('product.management.commands.fill_db.requests.get') as mock_requests:
            command = Command()
            mock_requests.side_effect = [MockResponseCat(), MockResponseProd()]
